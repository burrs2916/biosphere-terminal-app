...
export async function createConversation(agentId: string, title: string): Promise<ConversationDto> {
...
export async function runAgent(agentId: string, message: string, conversationId?: string): Promise<string> {
  return invoke('run_agent', { agentId, message, conversationId: conversationId ?? null });
}

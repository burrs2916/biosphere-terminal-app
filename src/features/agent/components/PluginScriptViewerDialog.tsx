...
import { XIcon, CodeIcon, ClockCounterClockwiseIcon, PackageIcon, ArrowUpIcon, PlusIcon, MinusIcon, CaretDownIcon, CaretRightIcon } from '@phosphor-icons/react';
import type { PluginManifest, PluginTool, ChangelogEntry, ToolChange } from '../../../proto/plugin';
import { useTheme } from '@mui/material/styles';

interface PluginScriptViewerDialogProps {
  open: boolean;
  onClose: () => void;
...
function ChangelogPanel({ changelog, isDark }: { changelog: ChangelogEntry[]; isDark: boolean }) {
  ...
  return (
    <Box sx={{ maxHeight: 500, overflow: 'auto', '&::-webkit-scrollbar': { width: 4 } }}>
...
export function PluginScriptViewerDialog({ open, onClose, plugin }: PluginScriptViewerDialogProps) {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1 }}>
        ...
        <Tooltip title={t('agent.close', 'Close')} arrow>
        ...
        {activeTab === 0 && (
          tools.length === 0 ? (
            ...
            <Box sx={{ display: 'flex', gap: 1.5, height: '100%' }}>
              <Box sx={{ width: 160, flexShrink: 0, overflow: 'auto', '&::-webkit-scrollbar': { width: 3 } }}>
                {tools.map((tool, idx) => (
                  <Box
                    key={tool.name}
                    onClick={() => setActiveTab(0)}
                    sx={{
                      px: 1, py: 0.5, mb: 0.25, borderRadius: 1, cursor: 'pointer',
                      bgcolor: idx === 0 ? (isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)') : 'transparent',
                      '&:hover': { bgcolor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.02)' },
                    }}
                  >
                    <Typography variant="caption" sx={{ fontSize: 11, fontWeight: 600, display: 'block' }} noWrap>
                      {tool.name}
                    </Typography>
                    <Typography variant="caption" sx={{ fontSize: 9, color: 'text.secondary', display: 'block' }} noWrap>
                      {tool.description}
                    </Typography>
                  </Box>
                ))}

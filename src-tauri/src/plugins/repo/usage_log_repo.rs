...
impl UsageLogRepo {
    ...
    pub fn clear_before(&self, before_ms: i64) -> Result<usize, String> {
        let conn = self.db.conn();
        conn.execute(
            "DELETE FROM plugin_usage_logs WHERE created_at < ?1",
            params![before_ms],
        ).map_err(|e| e.to_string()).map(|n| n as usize)
    }

    pub fn clear_failed_before(&self, before_ms: i64) -> Result<usize, String> {
        let conn = self.db.conn();
        conn.execute(
            "DELETE FROM plugin_usage_logs WHERE success = 0 AND created_at < ?1",
            params![before_ms],
        ).map_err(|e| e.to_string()).map(|n| n as usize)
    }
    ...
    pub fn purge_all(&self) -> Result<usize, String> {
        let conn = self.db.conn();
        conn.execute("DELETE FROM plugin_usage_logs", []).map_err(|e| e.to_string()).map(|n| n as usize)
    }

    pub fn count_by_plugin(&self, plugin_id: &str) -> Result<i64, String> {
        let conn = self.db.conn();
        conn.query_row(
            "SELECT COUNT(*) FROM plugin_usage_logs WHERE plugin_id = ?1",
            params![plugin_id],
            |row| row.get(0),
        ).map_err(|e| e.to_string())
    }
    ...
    pub fn count_all(&self) -> Result<i64, String> {
        let conn = self.db.conn();
        conn.query_row("SELECT COUNT(*) FROM plugin_usage_logs", [], |row| row.get(0)).map_err(|e| e.to_string())
    }

    pub fn total_size_estimate(&self) -> Result<i64, String> {
        let conn = self.db.conn();
        conn.query_row(
            "SELECT COALESCE(SUM(LENGTH(id) + LENGTH(plugin_id) + LENGTH(tool_name) + LENGTH(params_summary) + LENGTH(source) + LENGTH(COALESCE(error_message,'')) + LENGTH(COALESCE(output_summary,''))), 0) FROM plugin_usage_logs",
            [],
            |row| row.get(0),
        ).map_err(|e| e.to_string())
    }

    pub fn export_by_plugin(&self, plugin_id: &str) -> Result<Vec<UsageLogEntry>, String> {
        let conn = self.db.conn();
        let mut stmt = conn.prepare(
            "SELECT id, plugin_id, tool_name, params_summary, source, success, duration_ms, error_message, output_summary, created_at
             FROM plugin_usage_logs WHERE plugin_id = ?1 ORDER BY created_at ASC"
        ).map_err(|e| e.to_string())?;
    ...
    pub fn export_all(&self) -> Result<Vec<UsageLogEntry>, String> {
        let conn = self.db.conn();
        let mut stmt = conn.prepare(
            "SELECT id, plugin_id, tool_name, params_summary, source, success, duration_ms, error_message, output_summary, created_at
             FROM plugin_usage_logs ORDER BY created_at ASC"
        ).map_err(|e| e.to_string())?;
